def fun_with_ids
  ...
  puts "a_outer is #{a_outer} [42] with an id of: #{a_outer_id} BEFORE and: #{a_outer.object_id} AFTER the method call."
  puts "b_outer is #{b_outer} ['forty two'] with an id of: #{b_outer_id} BEFORE and: #{b_outer.object_id} AFTER the method call."
  puts "c_outer is #{c_outer} [42] with an id of: #{c_outer_id} BEFORE and: #{c_outer.object_id} AFTER the method call."
  puts "d_outer is #{d_outer} [42] with an id of: #{d_outer_id} BEFORE and: #{d_outer.object_id} AFTER the method call."
  puts
...
def an_illustrative_method(a_outer, b_outer, c_outer, d_outer, a_outer_id, b_outer_id, c_outer_id, d_outer_id)
  ...
  puts "b_outer id was #{b_outer_id} [60] before the method and is: #{b_outer.object_id} [60] inside the method."
  puts "c_outer id was #{c_outer_id} [80] before the method and is: #{c_outer.object_id} [80] inside the method."
